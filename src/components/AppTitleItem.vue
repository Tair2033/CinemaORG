<template>
  <div @click="goToPage" class="title-item">
    ...
    <div class="title-counter" v-if="currentPage > 1">
      {{ id + 1 + 10 * currentPage }}
    </div>
    ...
    <div class="title-about">
      ...
      <div class="title-about__right">
        ...
        <div class="title-about__button">
          <div class="bookmarks" v-if="!status" @click="status = true">
            Буду смотреть
          </div>
          <div class="watching" v-if="status">Вы смотрите &#10004;</div>
        </div>
      </div>
    </div>
  </div>
...
<script>
  export default {
    data() {
      return {
        status: false,
        genres: [...this.item.genres.map((i) => i.genre)].join(', ')
      };
    },
    methods: {
      goToPage(e) {
...
<style scoped>
  ...
  .watching {
    ...
    transition: trasnform 0.2s ease, background-color 0.2s ease;
  }
